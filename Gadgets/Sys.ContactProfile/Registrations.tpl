...
<script>  
  ...
  function getTableRow(item) {
    ...
    return render(
      '<tr data-registration-id="{{registrationId}}" ' + (item.isPendingWizardOperation ? '' : 'title="<$Text.ViewEventRegistrationDetailsTooltip$>" ') +
      ...
      '<td class="rightAlign lastTd">' +
        '<div data-is-waitList="{{isWaitList}}" data-registration-id="{{registrationId}}" data-is-past="{{isPast}}">{{status}}</div>' +
...
<script>
   renderTable();
   BonaPage.addPageStateHandler(BonaPage.PAGE_LOADED, function() {
   WA.WaitlistCancellationDialog( 
     "<$Text.ConfirmRemovalFromWaitlistDialogTitle$>",
     "<$Text.CloseButtonText$>",
     "<$Text.LeaveWaitListButtonText$>",
     "<$Text.WaitListConfirmationQuestion$>",
     "<$Text.RemoveFromWaitListLink$>",
   ) }, BonaPage.HANDLERTYPE_ALWAYS);
</script>
